<template>
  <div class="page"><PageHeader icon="🧰" title="硬件巡检" />
    ...
    <section v-if="comparison.overview.length"><div class="section-title"><h2>巡检摘要</h2><button class="download-btn" @click="downloadReport">下载完整报告</button></div><ul><li v-for="line in comparison.overview" :key="line">{{line}}</li></ul></section>
...
function reportRows(items){return items.map(item=>[item.name,...resultHosts.value.map(host=>item.values?.[host]||item.value||'未采集'),item.same===undefined?'':(item.same?'相同':'差异')])}
function tabulate(headers, rows){return [headers,...rows].map(row=>row.map(value=>String(value).replace(/\n/g,' | ')).join('\t')).join('\n')}
function downloadReport(){
  const hosts=resultHosts.value
  const sections=[]
  sections.push('硬件巡检完整报告')
  sections.push(`生成时间：${new Date().toLocaleString('zh-CN', { hour12:false })}`)
  sections.push(`主机：${hosts.join(', ')}`)
  sections.push('')
  sections.push('== 巡检摘要 ==', ...(comparison.value.overview||[]), '')
  if(comparison.value.same?.length) sections.push('== 节点相同配置 ==', tabulate(['项目','一致值'], comparison.value.same.map(item=>[item.name,item.value])), '')
  if(comparison.value.different?.length) sections.push('== 节点差异 ==', tabulate(['项目',...hosts], reportRows(comparison.value.different).map(row=>row.slice(0,-1))), '')
  if(comparison.value.nvme?.length) sections.push('== NVMe PCIe 设备对照 ==', tabulate(['设备',...hosts,'结论'], reportRows(comparison.value.nvme)), '')
  if(comparison.value.nics?.length) sections.push('== 物理网卡驱动/固件对照 ==', tabulate(['接口',...hosts,'结论'], reportRows(comparison.value.nics)), '')
  if(comparison.value.bandwidth?.length) sections.push('== PCIe 链路理论带宽对照 ==', '说明：实际 PCIe 链路（速率/通道）/ 单向理论带宽；不等同于 fio 或 iperf3 实测带宽。', tabulate(['设备',...hosts,'结论'], reportRows(comparison.value.bandwidth)), '')
  sections.push('== PCIe 标准单向理论带宽参考 ==', 'PCIe 3.0 (8 GT/s)：x1 0.98 GB/s；x4 3.94 GB/s；x8 7.88 GB/s；x16 15.75 GB/s', 'PCIe 4.0 (16 GT/s)：x1 1.97 GB/s；x4 7.88 GB/s；x8 15.75 GB/s；x16 31.51 GB/s', 'PCIe 5.0 (32 GT/s)：x1 3.94 GB/s；x4 15.75 GB/s；x8 31.51 GB/s；x16 63.02 GB/s', '')
  results.value.forEach(item=>sections.push(`== 单机原始报告：${item.host}（${item.status}） ==`, item.report||item.error||'未采集', ''))
  const blob=new Blob([sections.join('\n')],{type:'text/plain;charset=utf-8'})
  const link=document.createElement('a'); link.href=URL.createObjectURL(blob); link.download=`硬件巡检报告_${new Date().toISOString().replace(/[:T]/g,'-').slice(0,19)}.txt`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(link.href)
}
</script><style scoped>
...
.section-title { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; border-bottom: 2px solid #f0f0f0; margin-bottom: 15px; } h2 { font-size: 18px; font-weight: 600; color: #333; margin: 0; padding-bottom: 10px; } .download-btn { width: auto; padding: 7px 13px; margin: -1px 0 8px; font-size: 13px; white-space: nowrap; }
...
@media (max-width: 700px) { .page { padding: 12px; } .row { grid-template-columns: 1fr; } section { padding: 15px; } .section-title { align-items: center; } }
</style>
